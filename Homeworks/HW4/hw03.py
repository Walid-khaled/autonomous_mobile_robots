...
class EKFLocalization():
    def __init__(self, A, B, dt, std_vel, std_steer, dim_x, dim_z, sensor_size):
        self.dt = dt
    ...
    def run_localization(self, u, landmarks, std_range, std_bearing, step, ellipse_step, ylim, iteration_num):
        self.x = array([self.B]).T # x, y, rotation 
        ...
        plt.scatter(self.B[0],self.B[1],s=150, color='r', label='B')
        ...
        for i in range(iteration_num):
            ...
            track_pos.append(self.x)
            if i == iteration_num-1: print("reached final point A: \n", np.round(sim_pos[:2].T)[0]) 

        track = np.array(track)
        track_pos = np.array(track_pos) 
        plt.plot(track[:, 0], track[:,1], color='k', lw=2, label='traversed trajectory')
        plt.plot(track_pos[:, 0], track_pos[:,1], color='m', lw=2, label='uncertainty estimation')
        plt.axis('equal')
        plt.title("EKF Robot localization", fontsize=16)
...
ekf = EKFLocalization(A, B, dt, std_vel=5.1, std_steer=np.radians(1), dim_x=3, dim_z=landmarks.shape[0]*2, sensor_size=2)
ekf.run_localization(u, landmarks, std_range=0.3, std_bearing=0.1, step=10, ellipse_step=200, ylim=None, iteration_num=N)